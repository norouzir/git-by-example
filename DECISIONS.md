...
## 2026-09-19. Reflog defaults, a report to Git, and Parts 0 and 1

Three questions put to the author after the recorded defects were fixed.

**The reflog's expiry defaults are described as Git applies them.** Since Git
2.50 the code has the two defaults the other way round from the documentation,
which in effect makes every entry expire after 30 days. Chapter 36 gives both
sets with the versions each applies to, the practical rule that holds in every
version (at least 30 days), and how to set both values so every version agrees;
Chapters 23, 28 and 30 say "30 days, in every version". This follows the rule
of 2026-09-17, that the book describes the Git a reader has. It cannot be a
transcript, because the defaults count from the real clock, so the chapter says
how and when it was tested instead. **Rejected:** keeping the documented numbers
with a warning, which leaves a table that is false for the reader's Git; and
giving only Git 2.55's numbers, which misleads readers on older distributions.

An earlier message to the author said the swap gives recovery after a rebase
more time. Testing before writing showed it does not: the 30-day rule for every
entry is checked first. The chapter says what the test showed.

**The bug is reported to the Git project.** The author agreed. The report,
with the commit that introduced the swap (`85658275702b`) and a reproduction,
was drafted for the author to send from their own address to
git@vger.kernel.org; sending it is theirs. `SubmittingPatches` says the project
rejects contributions that look generated by AI, so the draft is short and
plain, and the author should read it and send it in their own words.

**Parts 0 and 1 are brought to the standard in the middle form.** Offered three
forms: the full command-chapter order, which gives a synopsis and options table
to chapters that have neither; nothing beyond the automatic checks; or the
middle. The author chose the middle: a question list for every section, which
Appendix G will also need; the manual checklist; examples where a reader would
still be unsure; and each chapter kept in its own order as a chapter about an
idea. Chapter 3 stays short and points to Chapter 62.

**What doing it found.** Beyond the question lists, most of the work was in
things no tool had caught: commands run out of sight inside a block, in
Chapters 5, 7 and 8, including the `git commit -a` of the section about it;
statements no one had tested, such as a branch named `@` being "ambiguous" (it
is not; `@` always means `HEAD`), downgrading Git being "equally safe" (not with
a repository extension the older Git lacks), and Git committing "silently" with
a guessed email (it prints a notice); and a Rust call-out that named only how
to turn on what Git 2.55 already turns on. CLAUDE.md now warns about the hidden
commands, since the verifier cannot see them.
